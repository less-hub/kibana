...
export const RightDetailsPanel: React.FC<RightPanel> = React.memo(() => {
  const {
    closePanels,
    closeRightPanel,
    openPanels,
  ...
  return (
    <EuiFlexGroup direction="column" css={{ backgroundColor: '#07C', height: '100%' }}>
      ...
      <EuiFlexItem grow={false}>
        ...
        >
          <FormattedMessage id="right.right2" defaultMessage="Open right2 panel" />
        </EuiButton>
      </EuiFlexItem>
      <EuiFlexItem grow={false}>
        <EuiButton
      ...
      <EuiFlexItem grow={false}>
        <EuiButton onClick={() => closePanels()}>
          <FormattedMessage id="right.close" defaultMessage="Close all panels" />
        </EuiButton>
      </EuiFlexItem>
    </EuiFlexGroup>
  );
